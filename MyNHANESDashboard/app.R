...
#Here I define  the Dashboard User Interface
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("analysis_type", "Select Analysis Type:",
                  choices = list(
                    ...
                    "BMI by Gender" = "bmi_gender",# '1'==male, '2' ==female
                    "Smoking vs Obesity" = "smoking_obesity",
                    "Race vs Education" = "race_education"
                  )),#The conditional panel allows the user to use drop down to select the analysis of their choice an the browse through plots and test statistics 
      conditionalPanel(
    ...
    mainPanel(# This is designs the main panel where I user can have a glance of their analyses and visualizations of the data once they have chosen the analysis type they are interested in. 
      tabsetPanel(
...
# Define Server, since we would like to interactively explore our analysses 
server <- function(input, output) {
  ...
  # Main visualization Plot that will be displayed on our dashboard from our analyses through scatter plots to show the association betweeb variables, box plots for comparisons,histograms and Q-Q plots for data distribution.
  output$main_plot <- renderPlot({
    ...
    if (input$analysis_type == "bp_analysis") {
      p1 <-ggplot(data, aes(x = BPXOSY1, y = BPXOSY2)) +
        ...
        labs(title = "SBP: 1st vs 2nd Readings Scatter plot",
      ...
      p2 <-ggplot(data, aes(x = log(BPXOSY1))) +
        geom_histogram(fill = "slateblue", col = "white",
                       bins = 20) + 
        labs(title = "Log-SBP 1st Reading distribution`",
      ...
      p3 <-ggplot(data, aes(x = log(BPXOSY2))) +
        geom_histogram(fill = "orange", col = "white",
                       bins = 20) + 
        labs(title = "SBP 2nd Reading Distribution",
      ...
      p4 <-ggplot(data, aes(x = "", y = log(BPXOSY1))) +
        ...
        coord_flip() + 
        labs(title ="Comparison Boxplot", y = "log(SBP 1st-Reading (mm Hg))")+
        theme_classic(base_size = 10) +
        theme(
          axis.text = element_text(face = "bold",size = 10),
          axis.title.y = element_text(face = "bold", size = 10),
      ...
      p1 +p2 +p3 +p4 # using the patchwork package's to display plots on the same panel
    } else if (input$analysis_type == "smoking_obesity") {
  ...
  # Summary Table to take a lot at our data summaries 
  ...
  # Statistical Results for our data statistical analyses
